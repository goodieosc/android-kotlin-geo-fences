...
class HuntMainActivity : AppCompatActivity() {
    ...
    //Receive pending intent from the broadcast receiver for when a geofence is triggered.
 ...
 */
    ...
    private fun addGeofenceForClue() {
        ...
        //check if we have any active geofences for our treasure hunt. If we already do, we shouldn't add another.
        if (viewModel.geofenceIsActive()) return

        //Find out the currentGeofenceIndex using the viewModel. If the index is higher than the number of landmarks we have,
        // it means the user has found all the treasures. Remove geofences call geofenceActivated on the viewModel, then return.
        ...
        //all removeGeofences() on the geofencingClient to remove any geofences already associated to the pending intent
